...
const ENTRIES = [
  ...
  ['night-reaver',     'Night Reaver',               ['cosmetic','dark'],              'wings-drakan',      'aura-vampyrism',    'twoh', 'dis-night-reaver-halberd'],
  ['midnight-slayer',  'Midnight Slayer',            ['cosmetic','dark'],              'wings-bloodblade',  'aura-vampyrism',    'twoh', 'dis-midnight-slayer-halberd'],
  ['celestial-slayer', 'Celestial Slayer',           ['cosmetic','prestige'],          'wings-salvation',   'aura-berserker',    'twoh', 'dis-celestial-slayer-halberd'],
  ...
  ['grave-guardian',   'Grave Guardian',             ['cosmetic','dark'],              'wings-decaying',    'aura-vampyrism',    'twoh', 'dis-grave-guardian-2h-sword'],
  ['ossified-guardian','Ossified Guardian',          ['cosmetic','dark'],              'wings-decaying',    'aura-vampyrism',    'twoh', 'dis-ossified-guardian-2h-sword'],
  ['sanguine-guardian','Sanguine Guardian',          ['cosmetic','dark'],              'wings-bloodblade',  'aura-vampyrism',    'twoh', 'dis-sanguine-guardian-2h-sword'],
  ['exiled-guardian',  'Exiled Guardian',            ['cosmetic','dark'],              'wings-drakan',      'aura-vampyrism',    'twoh', 'dis-exiled-guardian-2h-sword'],
  ['crimson-guardian', 'Crimson Guardian',           ['cosmetic','dark'],              'wings-bloodblade',  'aura-berserker',    'twoh', 'dis-crimson-guardian-2h-sword'],
...
// ── Build lookbook entry strings ──
// NOTE: '2h' is not a valid JS identifier so it must be quoted as a key.
// We use 'twoh' as an internal marker and emit it as quoted '2h' in output.
function makePiecesStr(setKey, cape, aura, wSlot, wId) {
  const parts = [
    `helm:'${setKey}-helm'`,
    `body:'${setKey}-body'`,
    `legs:'${setKey}-legs'`,
  ];
  if (cape) parts.push(`cape:'${cape}'`);
  if (aura) parts.push(`aura:'${aura}'`);
  if (wSlot && wId) {
    const key = wSlot === 'twoh' ? "'2h'" : wSlot;
    parts.push(`${key}:'${wId}'`);
  }
  return parts.join(', ');
}

const newEntries = ENTRIES.map(([setKey, name, themes, cape, aura, wSlot, wId]) => {
  const themesStr = JSON.stringify(themes); // ["cosmetic","dark"] — valid JS
  const piecesStr = makePiecesStr(setKey, cape, aura, wSlot, wId);
  return `  { name: ${JSON.stringify(name)}, theme:${themesStr}, previewSetKey:'${setKey}',\n    pieces:{ ${piecesStr} } }`;
});

// ── Splice into LOOKBOOKS array (safe block-scoped approach) ──
...
const scriptStart = newHtml.indexOf('<script>') + 8;
const scriptEnd   = newHtml.indexOf('</script>', scriptStart);
const scriptSrc   = newHtml.slice(scriptStart, scriptEnd);
try {
  new Function(scriptSrc);
  console.log('Syntax check passed ✓');
} catch (e) {
  console.error('SYNTAX ERROR — aborting:', e.message);
  process.exit(1);
}
